...
/**
 * Finds the root directory based on the project mode.
 * @param {ProjectMode} mode - Either "consumer" or "self"
 * @returns {string} - Absolute path to the project root
 */
function findRootByMode(mode) {
  ...
  while (true) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (mode === "self" && pkg["smart-i18n-library"] === true) {
          return currentDir;
        }
        if (mode === "consumer" && pkg["smart-i18n-library"] !== true) {
          return currentDir;
        }
      } catch {
    ...
    if (parentDir === currentDir) {
      throw new Error(`❌ Could not find a valid project root for mode "${mode}".`);
    }
 ...
 */
function joinRootPathByMode(mode, segments) {
...
export function getConsumerRoot() {
  return findRootByMode("consumer");
}
